...
void Deposit::on_pushButton_res_clicked() {
  ...
  if ((check_summ + check_interest + check_n + check_add + check_sub +
    ...
    controller.CalculateDeposit(summ, n, interest_rate, tax_rate,
                                periodicity_payments, capitalization, count_add,
                                add, count_sub, sub, this);
  }
}

void Deposit::on_clear_Button_clicked() {
...
void Deposit::SetResult(double result) {
  ui->label_res->setText(QString::number(result, 'f', 7));
}
void Deposit::SetTaxWith(double tax_with) {
  ui->label_tax->setText(QString::number(tax_with, 'f', 7));
}
void Deposit::SetAccruedInterest(double accrued_interest) {
