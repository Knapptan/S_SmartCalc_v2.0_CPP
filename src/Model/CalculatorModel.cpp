...
// разбивает входное арифметическое выражение на токены
void CalculatorModel::Tokenize(const std::string& expression) {
  if (expression.length() > 255) {
    throw std::logic_error("Expression is too long");
  }
  if (!CheckBrackets(expression)) {
    throw std::logic_error("Error brackets in expression");
  }
...
std::string CalculatorModel::toLowerCase(const std::string& str) {
  std::string result;
  for (char c : str) {
...
// считывает последовательность букв из строки, начиная с определенного индекса
std::string CalculatorModel::ReadWord(const std::string& expression,
                                      size_t& start_index) {
...
// считывает числовое значение из строки, начиная с определенного индекса
double CalculatorModel::ReadNumber(const std::string& expression,
                                   size_t& start_index) {
  std::regex double_regex("\\d+([.]\\d+)?(e([-+])?\\d+)?");
  std::sregex_iterator regex_iterator = std::sregex_iterator(
      expression.begin() + start_index, expression.end(), double_regex);
  if (regex_iterator != std::sregex_iterator()) {
    std::smatch match = *regex_iterator;
    start_index += match.length();
...
// считывает числовое значение из строки, начиная с определенного индекса
std::string CalculatorModel::ReadSingleChar(const std::string& expression,
                                            size_t& start_index) {
...
// Метод для проверки правильности расположения скобок в выражении
bool CalculatorModel::CheckBrackets(const std::string& expression) {
  int bracket = 0;
  ...
  return !break_point && bracket == 0;
}

// пытается добавить токен в очередь ввода
void CalculatorModel::TryToPushToken(std::string token) {
...
// для модификации очереди токенов, включая определение унарных плюсов и минусов
void CalculatorModel::ModifyQueue() {
...
// для валидации очереди токенов на предмет соответствия ожидаемой
// последовательности токенов
void CalculatorModel::ValidateQueue() {
  std::queue<Token> tempQueue = input_;
  // проверки первого токена
  ...
  if (firstTokenType >= 0 && firstTokenType < kNumTokenTypes) {
    if (allowedTypesForFirst[firstTokenType] != 1) {
      ...
      throw std::logic_error("Invalid first token type: " +
                             std::to_string(firstTokenType));
    }
  } else {
    while (!input_.empty()) {
      input_.pop();
...
// для преобразования инфиксной нотации в постфиксную
void CalculatorModel::ConvertInfixToPostfix() {
  while (!input_.empty()) {
...
// для перемещения токенов между входной очередью, стеком и выходной очередью
void CalculatorModel::MoveTokenFromInputToOutput() {
  if (!input_.empty()) {
...
void CalculatorModel::MoveTokenFromInputToStack() {
  if (!input_.empty()) {
    Token token = input_.front();
    input_.pop();
    stack_.push(token);
...
void CalculatorModel::MoveTokenFromStackToOutput() {
  if (!stack_.empty()) {
...
// для вычисления значения арифметического выражения в постфиксной нотации
double CalculatorModel::PostfixExpressionCalculate(double x_value) {
  input_ = output_;
  while (!input_.empty()) {
...
// Методы для управления стеком результатов вычислений
void CalculatorModel::PushToResult(const function_variant& value) {
...
double CalculatorModel::PopFromResult() {
  if (!result_.empty()) {
...
// для получения результирующего значения
double CalculatorModel::GetAnswer() { return answer_; }

// для запуска процесса вычисления
void CalculatorModel::CalculateAnswer(const std::string& input_expression,
                                      const std::string& input_x) {
  size_t start_i = 0;
  double x = ReadNumber(input_x, start_i);

  // Проверка, что x - действительное число, и строка input_x не пуста
  if (!std::isnan(x) && !input_x.empty()) {
    Tokenize(input_expression);
    ModifyQueue();
    ValidateQueue();
    ConvertInfixToPostfix();
    PostfixExpressionCalculate(x);
  }
}

// для вычисления значения арифметического выражения при заданном значении
// переменной x
void CalculatorModel::GrafCalculateAnswer(const std::string& input_expression,
  ...
  if (!std::isnan(input_x)) {
    ...
    ValidateQueue();
    ConvertInfixToPostfix();
    PostfixExpressionCalculate(input_x);
  }
}
